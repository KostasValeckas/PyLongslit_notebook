...
def reduce_obs(xsize, 
    ...
    FLATframe = fits.open(out_dir + '/Flat.fits')
    FLAT = np.array(FLATframe[0].data)

    rawimages = sorted(frame_list)
    ...
    for n in range(0, n_rawimages):
        spec = fits.open(out_dir + "/" + rawimages[n])
        print('Info on file:')
        print(spec.info())
        ...
        print(outnames[n])
        fits.writeto(out_dir + "/" + outnames[n],specdata1,hdr,overwrite=True)

    #Add and rotate

    sum = np.zeros_like(fits.open(out_dir + "/" + outnames[0])[0].data)
    ...
    hduout.writeto(
        out_dir + "/obj.fits" if not standard_star_reduction else out_dir + "/std.fits",
        overwrite=True
    )
    ...
    hduout.header['CDELT2'] = 1
    hduout.writeto(
        out_dir + "/arcsub.fits" if not standard_star_reduction else out_dir + "/arcsub_std.fits",
        overwrite=True
    )
