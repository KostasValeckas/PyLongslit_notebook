...
from utils import fake_multispec_data

def calibrate(out_dir = None, extinction_file_path = None):

    #Read the sensitivity-solution
    try:
        coeff_ID = np.loadtxt(out_dir +'/sens_coeff.dat')
    except IOError:
        print("Output from sensfunction.py is not accessible in the database")

    #Read the spectrum to be calibrated
    try:
        file = fits.open(out_dir +"/obj.ms_1d.fits")
    except IOError:
        print("Missing input spectrum to calibrate?")

    hdr_obj = fits.open(out_dir + "/obj.fits")[0].header
    ...
    airmass = hdr_obj['AIRMASS']
    exptime = hdr_obj['EXPTIME']
    ...
    try:
        dataext = np.loadtxt(extinction_file_path)
    except IOError:
