...
def extract_1d_spec(
    ...
    objhdu = fits.open(out_dir+'/obj.fits')
    stdhdu = fits.open(out_dir+'/std.fits')
    lamphdu = fits.open(out_dir+'/arcsub.fits')
    stdlamphdu = fits.open(out_dir+'/arcsub_std.fits')
    ...
    #check for NaNs
    if np.isnan(objimage).any():
        print('There are NaNs in the object image. This may course a crash.')
    if np.isnan(stdimage).any():
        print('There are NaNs in the std image. This may course a crash.')
    if np.isnan(lampimage).any():
        print('There are NaNs in the arc image. This may course a crash.')
    if np.isnan(stdlampimage).any(): 
        print('There are NaNs in the std-arc image. This may course a crash.')
    ...
    # Calculating constants by using the loaded reduced images
    ...
    # Gain and read-out noise wiped in 'obj.fits' and 'subx.fits',
    #  using the values from the header of the reduced images.
    
    # Get all files starting with "crr" in out_dir
    pattern = os.path.join(out_dir, "crr*")

    # Use glob to find all files matching the pattern
    files_starting_with_crr = glob.glob(pattern)

    hdu_temp = fits.open(files_starting_with_crr[0])

    GAIN = hdu_temp[1].header['GAIN']
    RON = hdu_temp[1].header['RDNOISE']

    print("Got Gain: ", GAIN, " and Read-out noise: ", RON)
    ...
    if not standard_star:
        _.writeto((out_dir+"/obj.skysub.fits"), overwrite=True)
        _ = fits.PrimaryHDU(data=data_variance, header=hdr)
        _.data = _.data.astype('float32')
        _.writeto((out_dir+"/obj.variance.fits"), overwrite=True)
        _ = fits.PrimaryHDU(data=data_sky, header=hdr)
        _.data = _.data.astype('float32')
        _.writeto((out_dir+"/obj.sky.fits"), overwrite=True)
    else:
        _.writeto((out_dir+"/std.skysub.fits"), overwrite=True)
        _ = fits.PrimaryHDU(data=data_variance, header=hdr)
        _.data = _.data.astype('float32')
        _.writeto((out_dir+"/std.variance.fits"), overwrite=True)
        _ = fits.PrimaryHDU(data=data_sky, header=hdr)
        _.data = _.data.astype('float32')
        _.writeto((out_dir+"/std.sky.fits"), overwrite=True)
    ...
    if not standard_star:
        ...
        hduout.writeto(out_dir+"/obj.ms_1d.fits", overwrite=True)
        print(" ")
        print("** Wrote output file '%s.ms_1d.fits' ." % (out_dir+"/obj"))
        print(" ")
        ...
        df.to_csv(out_dir+"/obj.ms_1dw.dat", header=None, index=None, sep=' ')
        print(" ")
        print("** Wrote output file '%s.ms_1d.dat' ." % (out_dir+"/obj"))
        print(" ")

    else:

        hduout.writeto(out_dir+"/std.ms_1d.fits", overwrite=True)
        print(" ")
        print("** Wrote output file '%s.ms_1d.fits' ." % (out_dir+"/std"))
        print(" ") 

        #Also write spectrum to an ascii-file
        df_frame = {'wave':wavelength, 'optflux':out1, 'sumflux':out2, 'sky':out3, 'opt_sigma':out4}
        df = pd.DataFrame(df_frame,dtype='float32')
        df.to_csv(out_dir+"/std.ms_1d.dat", header=None, index=None, sep=' ')
        print(" ")
        print("** Wrote output file '%s.ms_1d.dat' ." % (out_dir+"/std"))
        print(" ")
