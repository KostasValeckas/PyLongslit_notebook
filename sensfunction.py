...
def sensfunc(out_dir = ".",
             extinction_file_path = None, 
             FITTING_MODEL_SF = 'Chebyshev', 
             ORDER_SF = 9):

    #Read output from standard.py
    try:
        data = np.loadtxt(out_dir + '/stdinfo')
        datastd = np.loadtxt(out_dir + '/stddata')
    except IOError:
        print("Output from standard.py is not accessible.")
    ...
    f = open(out_dir + '/sens_coeff.dat', 'w')
